...
<body id="kt_app_body" data-kt-app-layout="dark-sidebar" data-kt-app-header-fixed="true"
    ...
    @if (Auth::user()->sounds == 1 &&
            Auth::user()->role == 0 &&
            Route::currentRouteName() == 'student.single_package_course')
    ...
    <script>
        document.querySelectorAll('.toggle-status').forEach((checkbox) => {
            checkbox.addEventListener('change', function() {
                ...
                0; // Determine the new value (1 for checked, 0 for unchecked)
                ...
                fetch('student/update-user-status', {
                    ...
                    .then(data => {
                        if (data.success) {
                            const statusText = value === 1 ? 'enabled' :
                            'disabled'; // Determine the status text

                            toastr.success(
                            `${field} is now ${statusText}`); // Success message with "enabled" or "disabled"
                        } else {
                    ...
                    .catch(error => {
                        ...
                        toastr.error(
                        'An error occurred while updating status.'); // General error message
                    });
            });
        });
    </script>
